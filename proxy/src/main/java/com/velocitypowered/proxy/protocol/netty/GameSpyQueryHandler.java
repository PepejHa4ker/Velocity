...
public class GameSpyQueryHandler extends SimpleChannelInboundHandler<DatagramPacket> {
    ...
    private static final byte[] QUERY_RESPONSE_FULL_PADDING = new byte[]{
            0x73, 0x70, 0x6C, 0x69, 0x74,
            0x6E, 0x75, 0x6D, 0x00, (byte) 0x80, 0x00
    };
    private static final byte[] QUERY_RESPONSE_FULL_PADDING2 = new byte[]{
            0x01, 0x70, 0x6C, 0x61,
            ...
            case QUERY_TYPE_STAT: {
                ...
                server.getEventManager()
                      ...
                      .thenAcceptAsync((event) -> {
                          ...
                          responseWriter.writePlugins(
                                  event.getResponse().getProxyVersion(),
    ...
    private List<QueryResponse.PluginInformation> getRealPluginInformation() {
        ...
        for (PluginContainer plugin : server.getPluginManager().getPlugins()) {
            ...
            result.add(QueryResponse.PluginInformation.of(
                    description.getName()
                               .orElse(description.getId()),
